...
const Router = {
  ...
  async navigate(screenId, params = {}, addToHistory = true) {
    ...
    if (target) {
      ...
      // Show loading spinner while render runs
      if (!target.innerHTML || target.innerHTML.trim() === '') {
        target.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:#F8F7FF;"><div style="text-align:center;"><div style="width:48px;height:48px;border:3px solid #EDE9FF;border-top-color:#6C3AE8;border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto 16px;"></div><p style="color:#94A3B8;font-size:14px;">Loading…</p></div></div><style>@keyframes spin{to{transform:rotate(360deg)}}</style>';
      }
    }
...
const UI = {
  ...
  avatar(src, initials, sizeCls = 'md', colorClass = '') {
    if (src) {
      return `<img src="${src}" alt="${initials}" class="avatar avatar--${sizeCls}">`;
    }
    const colors = ['', 'style="background:linear-gradient(135deg,#6C3AE8,#A855F7)"',
    ...
    return `<div class="avatar-placeholder avatar-placeholder--${sizeCls}" ${colors[colorIdx]}>${initials}</div>`;
  },
